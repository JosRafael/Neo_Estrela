...
import PanelRoleta from "../DragonTigerPanel";
...
const DragonTiger = () => {
  ...
  const [mensagemAtual, setMensagemAtual] = useState(<span>Analisando padrões...</span>);
  ...
  const estrategias = [
    (tempoExpiracao) => (<span>Entrada confirmada no 🟡 Tiger até <HighlightedText>2 proteções</HighlightedText>, válido até {tempoExpiracao}</span>),
    (tempoExpiracao) => (<span>Entrada confirmada no 🔴 Dragon até <HighlightedText>2 proteções</HighlightedText>, válido até {tempoExpiracao}</span>)
  ];

  useEffect(() => {
    const messageSequence = [
        { message: "Estou analisando qual a melhor estratégia nesse momento 🤑", delay: 0 },
    ...
    const generateStrategy = () => {
      ...
      const tempoExpiracao = new Date(dataAtual.getTime() + 2 * 60 * 1000).toLocaleTimeString().slice(0, 5);
    ...
    const fetchData = async () => {
      try {
        await api.get('/games/ultimosNumeros', {
          headers: {
  ...
  return (
    <Container>
      ...
      <General>
        ...
        <Main>
          <MainMsg>
            <MainName>Vitoria Profaci</MainName>
            {isLoading ? (
      ...
      <IframeComponent link={LinkGame[3].link} />
      </Container>
  );
};
